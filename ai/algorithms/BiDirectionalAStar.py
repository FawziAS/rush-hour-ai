...
class BiDirectionalAStar:
    ...
    def start_bidirectional_a_star(initial_state_representation, goal_state_representation, heuristic, time_limit):
        ...
        goal_state = StateNode(goal_state_representation,
                               -1*heuristic.calculate_heuristic_value(goal_state_representation), 0)

        # Added initial states
        heapq.heappush(BiDirectionalAStar.opened_front, initial_state)
        BiDirectionalAStar.opened_front_dictionary.update(
            {initial_state_representation.get_board_str(): initial_state})
        heapq.heappush(BiDirectionalAStar.opened_back, goal_state)
        BiDirectionalAStar.opened_back_dictionary.update(
            {goal_state_representation.get_board_str(): goal_state})
        ...
        while BiDirectionalAStar.opened_front and BiDirectionalAStar.opened_back:
            ...
            if match_found is not None:
                print("Success")
                backwards_path = BiDirectionalAStar.backwards(match_found.get_state_representation().get_solution_path())
            ...
            BiDirectionalAStar.add_relevant_states(front_neighbours, BiDirectionalAStar.opened_front,
                                                   BiDirectionalAStar.opened_front_dictionary,
                                                   BiDirectionalAStar.closed_front, heuristic, front_state.get_depth())
            BiDirectionalAStar.add_relevant_states(back_neighbours, BiDirectionalAStar.opened_back,
                                                   BiDirectionalAStar.opened_back_dictionary,
                                                   BiDirectionalAStar.closed_back, BackwardsBlockingHeuristic, back_state.get_depth())
        print("Failed")
        return
    ...
    @staticmethod
    def add_relevant_states(neighbour_states, opened, opened_dictionary, closed, heuristic, previous_depth):
